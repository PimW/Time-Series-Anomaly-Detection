...
class TSOutlierDetection(object):
    ...
    def detect_outliers(self, occurrence_thr=0):
        ...
        return [slice(s, s + self.sax_model.window) for s in np.asarray(tswindow_index)[outlier_index]], \
               np.asarray(tswindow_occurrence)[outlier_index]
    ...
    def numerosity_reduce(self, symbols, method="exact"):
        if method is "exact":
            reduced_symbols = []
            indices = []
            grps = groupby(enumerate(symbols), lambda k_v: k_v[1])
            for (s, grp) in grps:
                grp = list(grp)
