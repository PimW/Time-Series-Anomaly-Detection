...
class SAXModel(object):
    def __init__(self, window=None, stride=None,
        ...
        vecs = [(a_b[0] + a_b[1]) / 2 for a_b in zip(cps, cps[1:])]  ## taking mean may not be accurate
        vecs[0] = cps[1]
        vecs[-1] = cps[-2]
        self.sym2vec = dict(list(zip(self.alphabet, vecs)))
    ...
    def paa_window(self, window_signal):
        ...
        xs = [np.sum(ss) / float(binsize) for ss in self.binpack(s)]
        return xs
    ...
    def symbolize_signal(self, signal):
        """
        ...
        return [self.symbolize_window(signal[wi]) for wi in window_index]
    ...
    def symbol_distance(self, word1, word2):
        ...
        diff = np.asarray([0 if abs(iw1 - iw2) <= 1 else cutpoints[max(iw1, iw2) - 1] - cutpoints[min(iw1, iw2)]
                           for (iw1, iw2) in
                           zip(list(map(inverted_alphabet.get, word1)), list(map(inverted_alphabet.get, word2)))])
